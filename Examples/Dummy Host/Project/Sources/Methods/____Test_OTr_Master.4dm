...
If (Current process name:C1392=$DesiredProcessName_t)
	...
	// vtCC_Filename is intentionally NOT declared with `var` — it must be a
	// genuine named process variable so that OTr_uTextToPointer can resolve it
	// via Get pointer("vtCC_Filename") inside the component context.
	...
	If (Records in table:C83([Demo:1])>0)
		...
		READ ONLY:C145(*)  // So we don't lock the record
		ALL RECORDS:C47([Demo:1])
		$testName_t:="OTr_GetRecordTable hand-crafted snapshot = 7"
		
		OTr_PutRecord($h5_i; "snaptag"; Table:C252(->[Demo:1]))
		$expected_t:=String:C10(Table:C252(->[Demo:1]))
		$actual_t:=String:C10(OTr_GetRecordTable($h5_i; "snaptag"))
		$pass_b:=($expected_t=$actual_t)
		$masterText_t:=$masterText_t+String:C10($rowNum_i)+$TAB+$phase_t+$TAB+$testName_t+$TAB+$expected_t+$TAB+$actual_t+$TAB+Choose:C955($pass_b; "Pass"; "FAIL")+$LF
		If ($pass_b)
			$totalPass_i:=$totalPass_i+1
		Else 
	...
	$expected_t:="size>0"
	$actual_t:="size="+String:C10(BLOB size:C605($serialBlob6_blob))
	$pass_b:=(BLOB size:C605($serialBlob6_blob)>0)
	$masterText_t:=$masterText_t+String:C10($rowNum_i)+$TAB+$phase_t+$TAB+$testName_t+$TAB+$expected_t+$TAB+$actual_t+$TAB+Choose:C955($pass_b; "Pass"; "FAIL")+$LF
	If ($pass_b)
		$totalPass_i:=$totalPass_i+1
	Else 
	...
	$expected_t:="handle>0"
	$actual_t:="handle="+String:C10($h6b_i)
	$pass_b:=($h6b_i>0)
	$masterText_t:=$masterText_t+String:C10($rowNum_i)+$TAB+$phase_t+$TAB+$testName_t+$TAB+$expected_t+$TAB+$actual_t+$TAB+Choose:C955($pass_b; "Pass"; "FAIL")+$LF
	If ($pass_b)
		$totalPass_i:=$totalPass_i+1
	...
	//TEXT TO DOCUMENT($sbFolderPath_t+$sbFileName_t; $sbTable_t; "UTF-8")
	...
	SHOW ON DISK:C922($masterFilePath_t; *)
Else 
