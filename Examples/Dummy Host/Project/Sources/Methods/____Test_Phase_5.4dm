...
If (Current process name:C1392=$DesiredProcessName_t)
	...
	//OTr_zLock
	//OB SET(<>OTR_Objects_ao{$h_i}; "snaptag"; New object("__tableNum"; 7; "SomeField"; "test-value"))
	//OTr_zUnlock
	...
	//Else 
	//$failed_i:=$failed_i+1
	//$failures_t:=$failures_t+"GetRecordTable hand-crafted snapshot"+Char(Carriage return)
	//End if 
	...
	//$total_i:=$total_i+1
	//OTr_zLock
	//OB SET(<>OTR_Objects_ao{$h_i}; "notasnap"; New object("x"; "y"))
	//OTr_zUnlock
	//If (OTr_GetRecordTable($h_i; "notasnap")=0)
	//$passed_i:=$passed_i+1
	//Else 
	//$failed_i:=$failed_i+1
	//$failures_t:=$failures_t+"GetRecordTable missing __tableNum should return 0"+Char(Carriage return)
	//End if 
