...
// ----------------------------------------------------
// Project Method: OTr_z_LogWrite (inLevel; inSource; inMessage)
...
If ($activeLevel_t#"off")
	$activeRank_i:=OTr_z_LogLevelToInt($activeLevel_t)
	If ($inLevel_t="debug")
		$levelRank_i:=2
	Else 
		$levelRank_i:=1
	...
	If ($levelRank_i<=$activeRank_i)
		$currentFileName_t:=OTr_z_LogFileName
		...
		$callStack_t:=""
		If ($inLevel_t="error")
			$callStack_t:=OTr_z_LogGetCallStack($inSource_t)
		End if 
