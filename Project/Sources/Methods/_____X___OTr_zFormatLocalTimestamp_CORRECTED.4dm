...
// ============================================================
// CORRECTED VERSION: OTr_zFormatLocalTimestamp
// Use this to replace OTr_zFormatLocalTimestamp.4dm
// ============================================================
...
$year_i:=Num(Substring($inGMTTimestamp_t; 1; 4))
$month_i:=Num(Substring($inGMTTimestamp_t; 6; 2))
$day_i:=Num(Substring($inGMTTimestamp_t; 9; 2))
$hour_i:=Num(Substring($inGMTTimestamp_t; 12; 2))
$minute_i:=Num(Substring($inGMTTimestamp_t; 15; 2))
$second_i:=Num(Substring($inGMTTimestamp_t; 18; 2))
$millisecond_i:=Num(Substring($inGMTTimestamp_t; 21; 3))

// Get the cached UTC offset (in seconds)
$offsetReal_r:=Storage.OTr.log.UTCOffset
$offsetSeconds_i:=Integer($offsetReal_r)

// Build a working date and time, then add the offset
$workingDate_d:=Date($year_i; $month_i; $day_i)
$workingTime_h:=Time($hour_i; $minute_i; $second_i)
...
$dayCount_i:=Int($totalSeconds_i/86400)
...
$hourOfTime_i:=Int($secondOfTime_i/3600)
$minuteOfTime_i:=Int(($secondOfTime_i-($hourOfTime_i*3600))/60)
...
$workingDate_d:=Date(1900; 1; 1)+$dayCount_i

// Extract the adjusted date components
$yearOfDate_i:=Year of($workingDate_d)
$monthOfDate_i:=Month of($workingDate_d)
$dayOfDate_i:=Day of($workingDate_d)

// Format as "YYYY-MM-DDTHH:MM:SS.mmm"
$localTimestamp_t:=String($yearOfDate_i; "0000")+"-"+String($monthOfDate_i; "00")+"-"+String($dayOfDate_i; "00")+"T"+String($hourOfTime_i; "00")+":"+String($minuteOfTime_i; "00")+":"+String($secondOfTime_i; "00")+"."+String($millisecond_i; "000")
