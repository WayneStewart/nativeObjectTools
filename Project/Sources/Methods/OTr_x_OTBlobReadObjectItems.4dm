...
// ----------------------------------------------------
// Project Method: OTr_x_OTBlobReadObjectItems (inBlob; ioOffset; inItemCount) --> Object
...
While (($item_i<=$inItemCount_i) & ($ok_b))
	$key_t:=OTr_x_OTBlobReadSerializedText($inBlob_blob; $ioOffset_ptr; False:C215)
	...
	Else 
		...
		Case of 
			: ($typeByte_i=114)  // embedded object
				$childCount_i:=OTr_x_OTBlobReadUInt32BE($inBlob_blob; $ioOffset_ptr)
				$child_o:=OTr_x_OTBlobReadObjectItems($inBlob_blob; $ioOffset_ptr; $childCount_i)
			...
			: ($typeByte_i=115)  // packed record
				$record_o:=OTr_x_OTBlobReadRecord($inBlob_blob; $ioOffset_ptr)
			...
			: ($typeByte_i=129)  // real
				$ioOffset_ptr->:=$ioOffset_ptr->+1
				$real_r:=OTr_x_OTBlobReadRealBE($inBlob_blob; $ioOffset_ptr)
				OB SET:C1220($result_o; $key_t; $real_r)
				OB SET:C1220($result_o; OTr_z_ShadowKey($key_t); Is real:K8:4)
				
			: ($typeByte_i=132)  // date
				$ioOffset_ptr->:=$ioOffset_ptr->+1
				$day_i:=OTr_x_OTBlobReadUInt16BE($inBlob_blob; $ioOffset_ptr)
				$month_i:=OTr_x_OTBlobReadUInt16BE($inBlob_blob; $ioOffset_ptr)
				$year_i:=OTr_x_OTBlobReadUInt16BE($inBlob_blob; $ioOffset_ptr)
			...
			: ($typeByte_i=139)  // time
				$ioOffset_ptr->:=$ioOffset_ptr->+1
				$timeSeconds_i:=OTr_x_OTBlobReadUInt32BE($inBlob_blob; $ioOffset_ptr)
			...
			: ($typeByte_i=158)  // BLOB
				If (OTr_x_OTBlobReadBlobPayload($inBlob_blob; $ioOffset_ptr; ->$blob_blob))
			...
			: ($typeByte_i=161)  // Unicode text
				$ioOffset_ptr->:=$ioOffset_ptr->+1
				$value_t:=OTr_x_OTBlobReadSerializedText($inBlob_blob; $ioOffset_ptr; True:C214)
			...
			: (($typeByte_i>=142) & ($typeByte_i<=162))
				$array_o:=OTr_x_OTBlobReadArray($inBlob_blob; $ioOffset_ptr; $typeByte_i)
