...
$total_i:=$total_i+1
OK:=1
If ((OTr_GetArrayLong($h_i; "longs"; 1)=100) & (OTr_GetArrayLong($h_i; "longs"; 5)=500) & (OK=1))
	$passed_i:=$passed_i+1
Else 
	$failed_i:=$failed_i+1
...
If ((OTr_uEqualBLOBs($result_blob; $testBlob_blob)) & (OK=1))
	$passed_i:=$passed_i+1
Else 
	$failed_i:=$failed_i+1
...
If ((OTr_uEqualPictures($result_pic; $testWombat_pic)) & (OK=1))
	$passed_i:=$passed_i+1
Else 
	$failed_i:=$failed_i+1
...
If (($result_ptr->=vtCC_Filename) & (OK=1))
	$passed_i:=$passed_i+1
Else 
	$failed_i:=$failed_i+1
