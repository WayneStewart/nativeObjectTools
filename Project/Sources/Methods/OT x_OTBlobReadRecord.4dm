...
// ----------------------------------------------------
// Project Method: OT x_OTBlobReadRecord (inBlob; ioOffset) --> Object
...
// written so OT GetRecord can restore into a current record.
//
// Access: Private
//
// Parameters:
...
If ($ioOffset_ptr#Null)
	...
	If (($offset_i+7)<BLOB size($inBlob_blob))
		...
		$offset_i:=$offset_i+2
		$recordNum_i:=OT x_OTBlobReadUInt32BE($inBlob_blob; ->$offset_i)
		$flags_i:=OT x_OTBlobReadUInt16LE($inBlob_blob; ->$offset_i)
		...
		While (($ok_b) & (($offset_i+3)<BLOB size($inBlob_blob)))
			$fieldNum_i:=OT x_OTBlobReadUInt16LE($inBlob_blob; ->$offset_i)
			$fieldType_i:=OT x_OTBlobReadUInt16LE($inBlob_blob; ->$offset_i)
				...
				Case of
					: ($fieldType_i=1)
						If (($offset_i+1)<BLOB size($inBlob_blob))
							$value_i:=OT x_OTBlobReadUInt16LE($inBlob_blob; ->$offset_i)
					...
					: ($fieldType_i=4)
						If (($offset_i+3)<BLOB size($inBlob_blob))
							$value_i:=OT x_OTBlobReadInt32LE($inBlob_blob; ->$offset_i)
							OB SET($record_o; $fieldKey_t; $value_i)
							OB SET($fields_o; String($fieldNum_i); $fieldKey_t)
							$fieldCount_i:=$fieldCount_i+1
							If ($fieldName_t#"")
					...
					: ($fieldType_i=6)
						If (($offset_i+7)<BLOB size($inBlob_blob))
							$value_r:=OT x_OTBlobReadRealLE($inBlob_blob; ->$offset_i)
					...
					: ($fieldType_i=8)
						If (($offset_i+7)<BLOB size($inBlob_blob))
							$year_i:=OT x_OTBlobReadUInt16LE($inBlob_blob; ->$offset_i)
					...
					: ($fieldType_i=10)
						If (($offset_i+3)<BLOB size($inBlob_blob))
							$textLen_i:=OT x_OTBlobReadInt32LE($inBlob_blob; ->$offset_i)
							If ($textLen_i<0)
								$textLen_i:=0-$textLen_i
							End if
							If (($offset_i+(($textLen_i*2)-1))<BLOB size($inBlob_blob))
								$value_t:=OT x_OTBlobReadUTF16LE($inBlob_blob; ->$offset_i; $textLen_i; True)
								OB SET($record_o; $fieldKey_t; $value_t)
								OB SET($fields_o; String($fieldNum_i); $fieldKey_t)
								$fieldCount_i:=$fieldCount_i+1
								If ($fieldName_t#"")
