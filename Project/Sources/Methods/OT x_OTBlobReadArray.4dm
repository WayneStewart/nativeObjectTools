...
// ----------------------------------------------------
// Project Method: OT x_OTBlobReadArray (inBlob; ioOffset; inTypeByte) --> Object
...
If ($ioOffset_ptr#Null:C1517)
	...
	If (($arrayType_i>=0) & (($ioOffset_ptr->+15)<=BLOB size:C605($inBlob_blob)))
		...
		$unused_i:=OT x_OTBlobReadUInt32BE($inBlob_blob; $ioOffset_ptr)
		$current_i:=OT x_OTBlobReadUInt16BE($inBlob_blob; $ioOffset_ptr)
		$handleSize_i:=OT x_OTBlobReadUInt32BE($inBlob_blob; $ioOffset_ptr)
			...
			Case of 
				: ($arrayType_i=14)  // ARRAY REAL
					If ($handleSize_i>=(($count_i+1)*8))
						...
						For ($index_i; 1; $count_i)
							...
							$real_r:=OT x_OTBlobReadRealBE($inBlob_blob; ->$offset_i)
				...
				: ($arrayType_i=15)  // ARRAY INTEGER
					If ($handleSize_i>=(($count_i+1)*2))
						...
						For ($index_i; 1; $count_i)
							...
							$long_i:=OT x_OTBlobReadInt16BE($inBlob_blob; ->$offset_i)
				...
				: ($arrayType_i=16)  // ARRAY LONGINT
					If ($handleSize_i>=(($count_i+1)*4))
						...
						For ($index_i; 1; $count_i)
							...
							$long_i:=OT x_OTBlobReadInt32BE($inBlob_blob; ->$offset_i)
							OB SET:C1220($array_o; String:C10($index_i); $long_i)
						End for 
					Else 
						$ok_b:=False:C215
				...
				: ($arrayType_i=17)  // ARRAY DATE
					If ($handleSize_i>=(($count_i+1)*6))
						...
						For ($index_i; 1; $count_i)
							...
							$day_i:=OT x_OTBlobReadUInt16BE($inBlob_blob; ->$offset_i)
							$month_i:=OT x_OTBlobReadUInt16BE($inBlob_blob; ->$offset_i)
							$year_i:=OT x_OTBlobReadUInt16BE($inBlob_blob; ->$offset_i)
				...
				: ($arrayType_i=19)  // ARRAY PICTURE
					If ($handleSize_i>=(($count_i+1)*4))
						...
						For ($allIndex_i; 0; $count_i)
							...
							$size_i:=OT x_OTBlobReadUInt32BE($inBlob_blob; ->$offset_i)
							If ($size_i>0)
								If (($allocatedOffset_i+$size_i)<=BLOB size:C605($inBlob_blob))
									If ($allIndex_i>0)
										$picEnd_i:=$allocatedOffset_i
										If (OT x_OTBlobReadWrappedPicture($inBlob_blob; $allocatedOffset_i; ->$picEnd_i; ->$picture_pic))
				...
				: ($arrayType_i=32)  // ARRAY TIME
					If ($handleSize_i>=(($count_i+1)*4))
						...
						For ($index_i; 1; $count_i)
							$offset_i:=$dataStart_i+($index_i*4)
							$timeSeconds_i:=OT x_OTBlobReadUInt32BE($inBlob_blob; ->$offset_i)
				...
				: ($arrayType_i=34)  // ARRAY TEXT / STRING stored as Unicode
					If ($handleSize_i>=(($count_i+1)*16))
						...
						For ($allIndex_i; 0; $count_i)
							...
							$len_i:=OT x_OTBlobReadUInt32BE($inBlob_blob; ->$offset_i)
							If ($len_i>0)
								$value_t:=OT x_OTBlobReadSerializedText($inBlob_blob; ->$allocatedOffset_i; True:C214)
							Else 
								$value_t:=""
							End if 
							If ($allIndex_i>0)
