...
// ----------------------------------------------------
// Project Method: OTr_x_OTBlobToObject (inBlob) --> Object
...
If (BLOB size:C605($inBlob_blob)>=21)
	If (($inBlob_blob{0}=255) & ($inBlob_blob{1}=255) \
		...
		If ($compact_b)
			$offset_i:=14
			OTr_x_OTBlobReadUInt16LE($inBlob_blob; ->$offset_i)
			$rootType_i:=OTr_x_OTBlobReadUInt32LE($inBlob_blob; ->$offset_i)
			$itemCount_i:=OTr_x_OTBlobReadUInt32LE($inBlob_blob; ->$offset_i)
		...
		Else 
			$offset_i:=12
			$version_i:=OTr_x_OTBlobReadUInt32BE($inBlob_blob; ->$offset_i)
			...
			If ($version_i=4352)  // 0x1100
				...
				If ($rootType_i=OT Is Object)
					$itemCount_i:=OTr_x_OTBlobReadUInt32BE($inBlob_blob; ->$offset_i)
					If ($itemCount_i>=0)
						$result_o:=OTr_x_OTBlobReadObjectItems($inBlob_blob; ->$offset_i; $itemCount_i)
