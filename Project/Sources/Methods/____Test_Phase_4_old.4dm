...
ARRAY TIME:C1223($timeArr_atime; 0)
...
If (Size of array:C274($longArr_ai)=5)
	If (($longArr_ai{1}=10) & ($longArr_ai{5}=50))
		$passed_i:=$passed_i+1
	Else 
		$failed_i:=$failed_i+1
		ALERT:C41("GetArray longint values mismatch")
	End if 
Else 
	$failed_i:=$failed_i+1
	ALERT:C41("GetArray longint size mismatch")
End if 
...
Else 
	$failed_i:=$failed_i+1
	ALERT:C41("GetArray text mismatch")
End if 
...
Else 
	$failed_i:=$failed_i+1
	ALERT:C41("PutArrayLong/GetArrayLong mismatch")
End if 
...
Else 
	$failed_i:=$failed_i+1
	ALERT:C41("PutArrayReal/GetArrayReal mismatch")
End if 
...
Else 
	$failed_i:=$failed_i+1
	ALERT:C41("PutArrayString/Text mismatch")
End if 
...
$total_i:=$total_i+1
OTr_PutArrayDate($h1_i; "dates"; 1; Date:C102("1/4/2026"))
OTr_PutArrayDate($h1_i; "dates"; 2; Date:C102("25/12/2026"))

If (Substring:C12(String:C10(OTr_GetArrayDate($h1_i; "dates"; 1); ISO date GMT:K1:10); 1; 7)="2026-04")
	$passed_i:=$passed_i+1
Else 
	$failed_i:=$failed_i+1
	ALERT:C41("PutArrayDate/GetArrayDate mismatch")
End if 
...
Else 
	$failed_i:=$failed_i+1
	ALERT:C41("PutArrayTime/GetArrayTime mismatch")
End if 
...
$total_i:=$total_i+1
OTr_PutArrayBoolean($h1_i; "flags"; 1; True:C214)
OTr_PutArrayBoolean($h1_i; "flags"; 2; False:C215)
OTr_PutArrayBoolean($h1_i; "flags"; 3; True:C214)

If ((OTr_GetArrayBoolean($h1_i; "flags"; 1)=True:C214) & (OTr_GetArrayBoolean($h1_i; "flags"; 2)=False:C215))
	$passed_i:=$passed_i+1
Else 
	$failed_i:=$failed_i+1
	ALERT:C41("PutArrayBoolean/GetArrayBoolean mismatch")
End if 
...
Else 
	$failed_i:=$failed_i+1
	ALERT:C41("SizeOfArray returned "+String:C10($size_i)+", expected 3")
End if 
...
Else 
	$failed_i:=$failed_i+1
	ALERT:C41("InsertElement failed")
End if 
...
Else 
	$failed_i:=$failed_i+1
	ALERT:C41("DeleteElement failed")
End if 
...
Else 
	$failed_i:=$failed_i+1
	ALERT:C41("FindInArray returned "+String:C10($foundIdx_i)+", expected 2")
End if 
...
ALERT:C41("Phase 4 Test Summary:\r"+"Total: "+String:C10($total_i)+"\r"+"Passed: "+String:C10($passed_i)+"\r"+"Failed: "+String:C10($failed_i))
