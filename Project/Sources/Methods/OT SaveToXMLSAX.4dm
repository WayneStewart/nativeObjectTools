...
// ----------------------------------------------------
// Project Method: OT SaveToXMLSAX (inObject {; inPrettyPrint}) --> Text
...
//
// The XML schema produced is identical to that of OT SaveToXML (the DOM
// equivalent), so the output of both methods is fully interchangeable and
// can be parsed by OT LoadFromXML / OT LoadFromXMLFile without distinction.
//
...
// is accepted for API symmetry with OT SaveToXML but has no effect on the
// text returned.
//
// Shadow-type keys (leafKey$type) are included or excluded according to
// OT IncludeShadowKey (default: True).
//
// Use OT SaveToXMLFileSAX to write directly to a named file without the
...
If ($valid_b)
	...
	// Delegate all SAX serialisation to OT SaveToXMLFileSAX.
	// SAX always produces compact output; inPrettyPrint has no effect.
	OT SaveToXMLFileSAX($inObject_i; $tmpPath_t)
