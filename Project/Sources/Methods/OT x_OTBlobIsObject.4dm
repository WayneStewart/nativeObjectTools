...
// ----------------------------------------------------
// Project Method: OT x_OTBlobIsObject (inBlob) --> Boolean
...
// Guy Algot 2026-04-15: Added diagnostic reporting on failure
// ----------------------------------------------------

#DECLARE($inBlob_blob : Blob)->$isLegacy_b : Boolean

var $blobComplete_t; $blobSubset_t : Text
...
If (BLOB size:C605($inBlob_blob)>=24)
	...
	$blobComplete_t:=BLOB to text:C555($inBlob_blob; UTF8 text without length:K22:17)
	$blobSubset_t:=Substring:C12($blobComplete_t; 1; 24)
	
	OTr_z_Error("BLOB 24 chars = "+$blobSubset_t; Current method name:C684)
Else 
