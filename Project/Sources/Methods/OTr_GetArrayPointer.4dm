...
// Returns:
//   $value_p : Pointer : Element value, or Nil on any failure
			...
			If ($arrayType_i=Pointer array:K8:23)
				If (($index_i>=0) & ($index_i<=$arrayObj_o.numElements))
					If (OB Is defined($arrayObj_o; String($index_i)))
						$value_p:=OTr_uTextToPointer($arrayObj_o[String($index_i)])
						OTr_zSetOK(1)
					Else
						OTr_zSetOK(0)
					End if
				...
				End if
			Else
				OTr_zError("Tag does not reference a Pointer array"; Current method name)
				OTr_zSetOK(0)
			End if
		Else
			OTr_zSetOK(0)
