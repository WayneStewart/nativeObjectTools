...
// ----------------------------------------------------
// Project Method: OT FindInArray (inObject; inTag; inValue {; inStart}) --> Longint
...
// If *inStart* is not specified or is zero, it defaults to 1. The text inValue is
// converted to the type appropriate for the array being searched. For example, for a
// *Longint *array or *Real *array, inValue is converted as if it where passed to the 4D
// *Num* command. The formats used to convert values are as follows:

// Array type Example inValue

// Boolean array "true" or "1" = true, "false" or "0" = false Date array
// String(!08/27/31!)
...
// The result of the command is the index of the first matching element, or -1 if no
// match is found.
...
If (OTr_z_IsValidHandle($inObject_i))
	If (OTr_z_ResolvePath(<>OTR_Objects_ao{$inObject_i}; $inTag_t; False:C215; ->$parent_o; ->$leafKey_t))
		If (OB Is defined:C1231($parent_o; $leafKey_t))
			...
			$type_i:=OTr_z_ArrayType($arrayObj_o)
			If ($type_i=-1)
			...
			Else 
				...
				Case of 
					...
					Else 
						OTr_z_Error("Array type not supported for Find in array"; \
							Current method name:C684)
				End case 
				
				If ($workPtr#Null:C1517)
					OTr_z_ArrayFromObject($arrayObj_o; $workPtr)
					...
					Case of 
						...
						: ($type_i=Date array:K8:20)
							$searchDate_d:=OTr_u_TextToDate($inValue_t)
							$result_i:=Find in array:C230($Work_ad; $searchDate_d; $startAt_i)
							
						: ($type_i=Time array:K8:29)
							$search_h:=OTr_u_TextToTime($inValue_t)
