...
// ----------------------------------------------------
// Project Method: OTr_x_OTBlobReadSerializedText (inBlob; ioOffset; inDropNull) --> Text
...
If ($ioOffset_ptr#Null:C1517)
	If (($ioOffset_ptr->+4)<=BLOB size:C605($inBlob_blob))
		$charCount_i:=OTr_x_OTBlobReadUInt32BE($inBlob_blob; $ioOffset_ptr)
		If ($charCount_i=0)
			$value_t:=""
		Else 
			$value_t:=OTr_x_OTBlobReadUTF16BE($inBlob_blob; $ioOffset_ptr; $charCount_i; $inDropNull_b)
		End if 
	End if 
End if 
