...
// ----------------------------------------------------
// Project Method: OTr_z_SortValidatePair ($params_o : Object) --> Boolean
...
// any failure; OTr_z_ResolvePath reports its own errors.
...
If (OTr_z_ResolvePath(<>OTR_Objects_ao{$handle_i}; \
	...
	If (OB Is defined:C1231($parent_o; $leafKey_t))
		...
		If ($isSortable_b)
			If (($dir_t=">") | ($dir_t="<") | ($dir_t="*"))
				...
				$ok_b:=True:C214
			Else 
				OTr_z_Error("Invalid direction code: "+$dir_t; \
					Current method name:C684)
				OTr_z_SetOK(0)
			End if 
		Else 
			OTr_z_Error("Array not sortable: "+$tag_t; \
				Current method name:C684)
			OTr_z_SetOK(0)
		End if 
	Else 
		OTr_z_Error("Tag not found: "+$tag_t; \
			Current method name:C684)
		OTr_z_SetOK(0)
	End if 
End if 
