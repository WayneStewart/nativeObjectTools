...
// ----------------------------------------------------
// Project Method: OTr_PutArrayString ($handle_i : Integer; \
...
// and $index_i must be in range 0..numElements.
		...
		If (OB Is defined($parent_o; $leafKey_t))
			...
			If (($arrayType_i=Text array:K8:16) | ($arrayType_i=String array:K8:15))
				If (($index_i>=0) & ($index_i<=$arrayObj_o.numElements))
					$arrayObj_o[String($index_i)]:=$value_t
					OTr_zSetOK(1)
				Else
				...
				End if
			Else
				OTr_zError("Tag does not reference a Text or String array"; Current method name)
				OTr_zSetOK(0)
			End if
		Else
